...
filtCloud(pcl::PointCloud<PointT>::Ptr &cloud);
...
main (int argc, char **argv)
{
    ...
    while (ros::ok()  && !viewer.wasStopped())
    {
        ...
        int num_pts=(int)cloud_ptr->points.size();
        printf("Number of points: %d\n",num_pts);
        for(int i=0;i<std::min(5,num_pts);i++){
          auto pt=cloud_ptr->points[i];
          printf("Print %dth point info: ",i);
          printf("x=%.2f,y=%.2f,z=%.2f || ",pt.x,pt.y,pt.z);
          printf("r=%d,g=%d,b=%d\n",pt.r,pt.g,pt.b);
        }
        cout<<endl;
